...
CnSBEWallet::CnSBEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing nsbe seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadNSBESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseNSBESeed_deprecated()) {
                    LogPrintf("%s: Updated nSBE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old nsbe seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of nsbe wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CnSBEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteNSBECount(nCountLastUsed);
    else if (!walletdb.ReadNSBECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CnSBEWallet::Lock()
{
    seedMaster = 0;
}

void CnSBEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CnSBEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToNSBE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CnSBEWallet::LoadMintPoolFromDB()
{
...
void CnSBEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CnSBEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CnSBEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->nsbeTracker->HasPubcoinHash(pMint.first)) {
...
bool CnSBEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToNSBE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to nsbeTracker which also adds to database
    pwalletMain->nsbeTracker->Add(dMint, true);
...
void CnSBEWallet::SeedToNSBE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CnSBEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CnSBEWallet::GenerateDeterministicNSBE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CnSBEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CnSBEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
