...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.nsbeTracker = unique_ptr<CnSBETracker>(new CnSBETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == NSBE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == NSBE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
