...
int nAnonymizeSombeAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "sombe" is a composite category enabling all SOMBE-related debug output
            if (ptrCategory->count(string("sombe"))) {
                ptrCategory->insert(string("obfuscation"));
                ptrCategory->insert(string("ibanx"));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "sombe.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "sombed.pid"));
