...
CnSBETracker::CnSBETracker(std::string strWalletFile)
{
...
CnSBETracker::~CnSBETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CnSBETracker::Init()
{
...
bool CnSBETracker::Archive(CMintMeta& meta)
{
...
bool CnSBETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CnSBETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CnSBETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CnSBETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CnSBETracker::GetSerialHashes()
{
...
CAmount CnSBETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CnSBETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CnSBETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CnSBETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CnSBETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CnSBETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CnSBETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CnSBETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CnSBETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CnSBETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CnSBETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CnSBETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CnSBETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CnSBETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CnSBETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CnSBETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CnSBETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dnsbe from DB\n", __func__, listDeterministicDB.size());
    }
