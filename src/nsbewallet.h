...
#ifndef SOMBE_NSBEWALLET_H
#define SOMBE_NSBEWALLET_H
...
public:
    CnSBEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicNSBE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToNSBE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SOMBE_NSBEWALLET_H
