...
public:
    ...
    bool WriteNSBESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadNSBESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadNSBESeed_deprecated(uint256& seed);
    bool EraseNSBESeed();
    bool EraseNSBESeed_deprecated();

    bool WriteNSBECount(const uint32_t& nCount);
    bool ReadNSBECount(uint32_t& nCount);
