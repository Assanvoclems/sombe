...
#if defined(NDEBUG)
#error "SOMBE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usbe) are considered zero fee (for relaying and mining)
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6) {
        ...
        if (sigs >= IBANTX_SIGNATURES_REQUIRED) {
...
{
    //Check to see if the nSBE is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 nSBE spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend nSBE without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : nSBE spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : nSBE spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : nSBE spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-nsbe");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that nSBE mints are not already known
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t mNodeCoins = nMasternodeCount * 2000000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isNSBEStake)
{
    int64_t ret = 0;
...
CBitcoinAddress addressExp1("SQZzqnSR6PXxagep1byLiRg9ZurCZ5KieQ");
CBitcoinAddress addressExp2("STQYdnNqKuEHXyNeeYhPQGGGdqHbXYwjpj");
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SOMBE
...
{
    RenameThread("sombe-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateNSBEMinted()
{
...
void RecalculateNSBESpent()
{
    ...
    while (true) {
        ...
        //Remove spends from nSBE supply
...
bool UpdateNSBESupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSBESupply(Params().Zerocoin_StartHeight());
    }

    //Track nSBE money supply in the block index
    if (!UpdateNSBESupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new nSBE supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s nSbeSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SBE to nSBE
...
/*
    ...
    Remove conflicting blocks for successful iBanX transaction locks
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // ----------- ibanTX transaction scanning -----------
    if (IsSporkActive(SPORK_3_IBANTX_BLOCK_FILTERING)) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent nSBE spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of nSBE serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: nSBE stake block is less than allowed start height", __func__);

    if (CNSbeStake* nSBE = dynamic_cast<CNSbeStake*>(stake)) {
        CBlockIndex* pindexFrom = nSBE->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with nSBE stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: nSBE stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(nSBE->GetValue()));
        if (nChecksum200 != nSBE->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, nSBE->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsNSBE() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked nSBE fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d nSBE mints and %d nSBE spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SOMBE: We use certain sporks during IBD, so check to see if they are
    ...
    // getaddr message mitigates the attack.
        ...
        ProcessMessageiBanTX(pfrom, strCommand, vRecv);
