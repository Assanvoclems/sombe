                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseiBanTX"))
        settings.setValue("bUseiBanTX", false);

    bool useiBanTX = settings.value("bUseiBanTX").toBool();
    if (fLiteMode) {
        ui->checkiBanTX->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useiBanTX = false;
    } else {
        ui->checkiBanTX->setChecked(useiBanTX);
        CoinControlDialog::coinControl->useiBanTX = useiBanTX;
    }

    connect(ui->checkiBanTX, SIGNAL(stateChanged(int)), this, SLOT(updateiBanTX()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkiBanTX->isChecked()) {
        recipients[0].useiBanTX = true;
        strFunds += " ";
        strFunds += tr("using iBanX");
    } else {
        recipients[0].useiBanTX = false;
    }
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_SBE, true));
        if (!ctx.isValid()) {
...
void SendCoinsDialog::updateiBanTX()
{
    QSettings settings;
    settings.setValue("bUseiBanTX", ui->checkiBanTX->isChecked());
    CoinControlDialog::coinControl->useiBanTX = ui->checkiBanTX->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid SOMBE address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
