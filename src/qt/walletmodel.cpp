...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered sombe address / amount:
...

    {
        ...
        if (recipients[0].useiBanTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("iBanX doesn't support sending values that high yet. Transactions are currently limited to %1 SBE.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useiBanTX);
        transaction.setTransactionFee(nFeeRequired);

        if (recipients[0].useiBanTX && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("iBanX doesn't support sending values that high yet. Transactions are currently limited to %1 SBE.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal sombe:URI (sombe:XyZ...?message=example)
            {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].useiBanTX) ? "ix" : "tx"))
...
static void NotifynSBEReset(WalletModel* walletmodel)
{
    qDebug() << "NotifynSBEReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifynSBEReset.connect(boost::bind(NotifynSBEReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifynSBEReset.disconnect(boost::bind(NotifynSBEReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->nsbeTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
