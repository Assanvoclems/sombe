...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SBE);
    ...
    if (!settings.contains("nAnonymizeSombeAmount"))
        settings.setValue("nAnonymizeSombeAmount", 1000);

    nAnonymizeSombeAmount = settings.value("nAnonymizeSombeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSombeAmount"))
        SoftSetArg("-anonymizesombeamount", settings.value("nAnonymizeSombeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sombe.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSombeAmount:
            return QVariant(nAnonymizeSombeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSombeAmount:
            nAnonymizeSombeAmount = value.toInt();
            settings.setValue("nAnonymizeSombeAmount", nAnonymizeSombeAmount);
            emit anonymizeSombeAmountChanged(nAnonymizeSombeAmount);
            break;
        case CoinControlFeatures:
