...
#include "ui_nsbecontroldialog.h"
...
std::set<std::string> NSbeControlDialog::setSelectedMints;
std::set<CMintMeta> NSbeControlDialog::setMints;

NSbeControlDialog::NSbeControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::NSbeControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
NSbeControlDialog::~NSbeControlDialog()
{
    delete ui;
}

void NSbeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void NSbeControlDialog::updateList()
{
...
void NSbeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void NSbeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelNSbe_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setNSbeControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> NSbeControlDialog::GetSelectedMints()
{
...
void NSbeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
