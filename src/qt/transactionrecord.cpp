...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->nsbeTracker->HasMintTx(hash))) {
            //nSBE stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeNSBE;
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_nSbe;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IsNSBEType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeNSBE:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_nSbe:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeNSBE || type == TransactionRecord::MNReward) {
