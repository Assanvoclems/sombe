...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "sombe.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "sombed.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SBE and nSBE money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 23513, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SBE/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SBE/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via iBanX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via iBanX) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, sombe, (obfuscation, ibanx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SBE/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SOMBE specific functionality (Masternodes, Zerocoin, iBanX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-sbestake=<n>", strprintf(_("Enable or disable staking functionality for SBE inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-nsbestake=<n>", strprintf(_("Enable or disable staking functionality for nSBE inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:23513"));
    ...
    strUsage += HelpMessageOpt("-backupnsbe=<n>", strprintf(_("Enable automatic wallet backups triggered after each nSBE minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-nsbebackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic nSBE backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizesombeamount=<n>     " + strprintf(_("Keep N SBE anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageGroup(_("iBanX options:"));
    strUsage += HelpMessageOpt("-enableibantx=<n>", strprintf(_("Enable iBanX, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-ibantxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), niBanTXDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 23512, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The SOMBE Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SOMBE is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enableibantx", fEnableiBanTX)) {
        if (SoftSetArg("-ibantxdepth", "0"))
            LogPrintf("AppInit2 : parameter interaction: -enableibantx=false -> setting -niBanTXDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SOMBE Core is shutting down."));
...
#endif
    // Make sure only a single SOMBE process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SOMBE Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //SOMBE: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //SOMBE specific: zerocoin and spork DB's
                ...
                // SOMBE: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateNSBEMinted();
                        RecalculateNSBESpent();
                    }
                    RecalculateSBESupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // SOMBE: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SOMBE Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SOMBE Core to complete") << "\n";
        ...
        zwalletMain = new CnSBEWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize nSBEWallet
        uiInterface.InitMessage(_("Syncing nSBE wallet..."));

        bool fEnableNSbeBackups = GetBoolArg("-backupnsbe", true);
        pwalletMain->setNSbeAutoBackups(fEnableNSbeBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->nsbeTracker->Init();
...
//    }
//
//    nAnonymizeSombeAmount = GetArg("-anonymizesombeamount", 0);
//    if (nAnonymizeSombeAmount > 999999) nAnonymizeSombeAmount = 999999;
//    if (nAnonymizeSombeAmount < 2) nAnonymizeSombeAmount = 2;

    fEnableiBanTX = GetBoolArg("-enableibantx", fEnableiBanTX);
    niBanTXDepth = GetArg("-ibantxdepth", niBanTXDepth);
    niBanTXDepth = std::min(std::max(niBanTXDepth, 0), 60);
    ...
    LogPrintf("niBanTXDepth %d\n", niBanTXDepth);
    LogPrintf("Anonymize SOMBE Amount %d\n", nAnonymizeSombeAmount);
    ...
    /* Denominations
       ...
       For example:
       1SBE+1000 == (.1SBE+100)*10
       10SBE+10000 == (1SBE+1000)*10
    */
